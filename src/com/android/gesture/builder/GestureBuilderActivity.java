        ...
        mGestureOverlayView.addOnGesturePerformedListener(new OnGesturePerformedListener() {
            ...
            public void onGesturePerformed(GestureOverlayView overlay, Gesture gesture) {
              ...
              //识别手势，返回一个类型为Prediction的列表  
                ArrayList<Prediction> list = sStore.recognize(gesture);  
                
                //04-03 15:19:31.650: D/ge(19948): name:back,score:4.541964714881477
                //04-03 15:19:31.650: D/ge(19948): name:佛珠,score:1.3154468733546836
                //04-03 15:19:31.650: D/ge(19948): name:forward,score:0.7329219202909947
                /**
                 * 此处自动排序，将匹配级别最大的放到第一个**/
                for(Prediction pp:list){
                    Log.d("ge", "name:"+pp.name+",score:"+pp.score);
                }
                Prediction pre = list.get(0);  
                //如果匹配度大于1，表示可以识别，否则提示无法识别   
                if(pre.score > 1){
                    //判断名字是否与手势库的名字相同  
                    if(pre.name.equals("七")){
                        Toast.makeText(GestureBuilderActivity.this, "Recognize the 7", Toast.LENGTH_SHORT).show();  
                    }else if(pre.name.equals("佛珠")){  
                        Toast.makeText(GestureBuilderActivity.this, "Recognize the 6", Toast.LENGTH_SHORT).show();  
                    }else if(pre.name.equals("back")){  
                        Toast.makeText(GestureBuilderActivity.this, "Recognize the back", Toast.LENGTH_SHORT).show();  
                    }else if(pre.name.equals("forward")){  
                        Toast.makeText(GestureBuilderActivity.this, "Recognize the forward", Toast.LENGTH_SHORT).show();  
                    }else{
                        Toast.makeText(GestureBuilderActivity.this, "Recognize the "+pre.name, Toast.LENGTH_SHORT).show();  
                    }
                }else{
                    Toast.makeText(GestureBuilderActivity.this, "Can't Recognize ", Toast.LENGTH_SHORT).show();  
                }  
